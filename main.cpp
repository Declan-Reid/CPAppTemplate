	...
	while (true) {
		...
		switch (event.type) {
			case EVENT_TOUCH:
				...
				{
					setPixel(event.data.touch_single.p1_x, event.data.touch_single.p1_y, RGB_TO_RGB565(0, 0, 0));
				...
				{
					line(old_p1_x, old_p1_y, event.data.touch_single.p1_x, event.data.touch_single.p1_y, RGB_TO_RGB565(0, 0, 0));
					old_p1_x = event.data.touch_single.p1_x;
					old_p1_y = event.data.touch_single.p1_y;
				}
